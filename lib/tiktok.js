...
import { createLeadBreakdown } from './leads.js'
import { isCustomRange, parseCustomDateRange } from './reportRange.js'
...
function getDateRange(range, client) {
  ...
  const customRange = parseCustomDateRange(range, endDate)

  if (customRange) {
    startDate.setTime(new Date(`${customRange.startDate}T00:00:00.000Z`).getTime())
    endDate.setTime(new Date(`${customRange.endDate}T00:00:00.000Z`).getTime())
  } else if (range === '7d') {
...
async function fetchTikTokReportRows({
  ...
  while (true) {
    ...
    const pageRows = Array.isArray(data?.data?.list) ? data.data.list : []
    rows.push(...pageRows)

    const pageInfo = data?.data?.page_info || {}
    const totalPage = Number(pageInfo.total_page || pageInfo.totalPage || 0)
    if (!pageRows.length || (totalPage && page >= totalPage) || pageRows.length < pageSize) {
      break
    }
...
async function fetchTikTokMetrics({ accessToken, advertiserId, start_date, end_date }) {
  let rows = []
  try {
    rows = await fetchTikTokReportRows({
      accessToken,
      advertiserId,
      start_date,
      end_date,
      ...
      metrics: [
        'spend', 'reach', 'impressions', 'clicks', 'ctr', 'cpc', 'conversion',
        'onsite_form', 'offline_form_events',
...
async function fetchTikTokDailyMetrics({ accessToken, advertiserId, start_date, end_date }) {
  let rows
  try {
    rows = await fetchTikTokReportRows({
      accessToken,
      ...
      metrics: [
        'spend', 'conversion', 'onsite_form', 'offline_form_events',
        'messaging_total_conversation_tiktok_direct_message',
        'messaging_total_conversation_instant_messaging_app'
      ],
      pageSize: 1000
    ...
    rows = await fetchTikTokReportRows({
      ...
      end_date,
      dimensions: ['stat_time_day'],
      metrics: ['spend', 'conversion', 'onsite_form'],
      pageSize: 1000
    })
  }
  ...
  return rows.map((row) => {
    ...
    const rawDate = dimensions.stat_time_day || row.stat_time_day || row.date || ''
    const date = String(rawDate).slice(0, 10)
    const spend = safeNum(metrics.spend)
    const leadBreakdown = createLeadBreakdown({
      formSubmissions: safeNum(metrics.onsite_form) + safeNum(metrics.offline_form_events),
      directMessages:
        safeNum(metrics.messaging_total_conversation_tiktok_direct_message) +
        safeNum(metrics.messaging_total_conversation_instant_messaging_app)
    })
    ...
    return {
      ...
      totalLeads: leadBreakdown.totalLeads,
      cpa: leadBreakdown.totalLeads > 0 ? spend / leadBreakdown.totalLeads : null
    }
  }).filter((row) => row.date)
}
...
function sumTikTokMetrics(metricsRows) {
  const totals = metricsRows.reduce(
    (acc, metrics) => {
      ...
      acc.reach += safeNum(metrics.reach)
      acc.impressions += safeNum(metrics.impressions)
      acc.clicks += safeNum(metrics.clicks)
      acc.conversions += safeNum(metrics.conversion)
      acc.formSubmissions += safeNum(metrics.onsite_form) + safeNum(metrics.offline_form_events)
      acc.directMessages +=
        safeNum(metrics.messaging_total_conversation_tiktok_direct_message) +
        safeNum(metrics.messaging_total_conversation_instant_messaging_app)
      acc.videoViews += safeNum(metrics.video_play_actions)
      acc.engagements += safeNum(metrics.likes) + safeNum(metrics.comments) + safeNum(metrics.shares)
      acc.likes += safeNum(metrics.likes)
      acc.comments += safeNum(metrics.comments)
      acc.shares += safeNum(metrics.shares)
      return acc
    },
    {
      spend: 0,
  ...
  return {
    ...
    video_play_actions: totals.videoViews,
...
export async function getTikTokData({ clientId, range = '30d', advertiserId = null, clientName = null } = {}) {
  ...
  const chunks = range === 'max' || isCustomRange(range)
    ? splitDateRange(start_date, end_date)
    : [{ start_date, end_date }]
  const chunkResults = []
  const daily = []
  ...
  for (const chunk of chunks) {
    ...
    let dailyRows = []
    try {
      dailyRows = await fetchTikTokDailyMetrics({
        accessToken,
        advertiserId: effectiveAdvertiserId,
        ...chunk
      })
  ...
  return {
    ...
    currencyCode: client?.currencyCode || 'SAR',
    conversionLabel: 'TikTok conversions',
    spend: safeNum(metrics.spend),
    reach: safeNum(metrics.reach),
    ...
    conversionBreakdown: {
      ...
      totalResults: safeNum(metrics.conversion)
    },
    engagements: safeNum(metrics.engagements),
    videoViews: safeNum(metrics.video_play_actions),
    engagementBreakdown: {
      likes: safeNum(metrics.likes),
      comments: safeNum(metrics.comments),
      shares: safeNum(metrics.shares),
      totalEngagements: safeNum(metrics.engagements),
      videoViews: safeNum(metrics.video_play_actions)
    },
