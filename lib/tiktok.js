...
import { parseCustomStartDate } from './reportRange.js'
...
function getDateRange(range, client) {
  ...
  const customStartDate = parseCustomStartDate(range, endDate)

  if (customStartDate) {
    startDate.setTime(new Date(`${customStartDate}T00:00:00.000Z`).getTime())
  } else if (range === '7d') {
...
export async function getTikTokData({ clientId, range = '30d', advertiserId = null, clientName = null } = {}) {
  ...
  const chunks = range === 'max' || parseCustomStartDate(range)
