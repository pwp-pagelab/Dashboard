...
async function fetchTikTokMetrics({ accessToken, advertiserId, start_date, end_date }) {
  ...
  try {
    rows = await fetchTikTokReportRows({
      ...
      metrics: ['spend', 'reach', 'impressions', 'clicks', 'ctr', 'cpc', 'conversion', 'video_play_actions', 'likes', 'comments', 'shares']
    })
  } catch {
    try {
      rows = await fetchTikTokReportRows({
        ...
        end_date,
        dimensions: ['advertiser_id'],
        metrics: ['spend', 'reach', 'impressions', 'clicks', 'ctr', 'cpc', 'conversion', 'video_play_actions']
      })
    } catch {
      rows = await fetchTikTokReportRows({
        accessToken,
...
async function fetchTikTokDailyMetrics({ accessToken, advertiserId, start_date, end_date }) {
  const rows = await fetchTikTokReportRows({
    accessToken,
    advertiserId,
    start_date,
    end_date,
  ...
  return rows.map((row) => {
    ...
    const conversions = safeNum(metrics.conversion)
