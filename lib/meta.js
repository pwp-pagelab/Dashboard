...
export function getMetaActionBreakdown(actions = []) {
...
function getMetaEngagementBreakdown(actions = []) {
  ...
  const saves = getActionValue(actions, [
  ...
  return {
    ...
    saves,
    videoViews,
    totalEngagements: Math.max(pageEngagement, postEngagement, postReactions + comments + shares + saves)
  }
}
  ...
  return rows
    ...
    .map((row) => {
      ...
      const leadBreakdown = createLeadBreakdown({
        formSubmissions: actionBreakdown.leads,
        directMessages: actionBreakdown.messagingConversations
      })
...
export async function getMetaData({
  ...
  if (!row) return null
  const actionBreakdown = getMetaActionBreakdown(row.actions)
  const engagementBreakdown = getMetaEngagementBreakdown(row.actions)
  const leadBreakdown = createLeadBreakdown({
    formSubmissions: actionBreakdown.leads,
    directMessages: actionBreakdown.messagingConversations
  })
  ...
  return {
    ...
    leadBreakdown,
    engagements: engagementBreakdown.totalEngagements,
    videoViews: engagementBreakdown.videoViews,
    engagementBreakdown,
