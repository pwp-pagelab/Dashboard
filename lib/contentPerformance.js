...
async function linkedinAds(account, ctx) {
  ...
  const result = await request(url('https://api.linkedin.com/rest/adAnalytics', {
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    const data = await request(url(`https://api.linkedin.com/rest/adAccounts/${account.accountId}/creatives`, {
...
async function publishedPosts(account, ctx) {
  ...
  if (account.platform === 'linkedin') {
    ...
    const posts = []
    for (let page = 0; page < MAX_PAGES; page++) {
      const data = await request(url('https://api.linkedin.com/rest/posts', {
  ...
  const results = await Promise.all(selected.map(async (account) => {
    ...
    const run = async (kind, adapter) => {
      ...
      try {
        ...
        let message = 'Unable to sync. Check the connected account, token expiry, and read permissions.'
        try {
