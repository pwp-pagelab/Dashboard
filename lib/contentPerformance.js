...
const tiktokTokenCache = new Map()
const liHeaders = () => ({
...
async function getTikTokBusinessContentAuth(clientId, ctx) {
  const cached = tiktokTokenCache.get(clientId)
  if (cached?.expiresAt > Date.now() + 60000) return cached

  const directToken = envFor('TIKTOK_CONTENT_ACCESS_TOKEN', clientId)
  const directBusinessId = envFor('TIKTOK_CONTENT_BUSINESS_ID', clientId)
  const refreshToken = envFor('TIKTOK_CONTENT_REFRESH_TOKEN', clientId)
  if (!refreshToken) return directToken && directBusinessId
    ? { accessToken: directToken, businessId: directBusinessId, expiresAt: Date.now() + 300000 }
    : null

  const clientKey = envFor('TIKTOK_CONTENT_CLIENT_ID', clientId) || process.env.TIKTOK_APP_ID
  const clientSecret = envFor('TIKTOK_CONTENT_CLIENT_SECRET', clientId) || process.env.TIKTOK_APP_SECRET
  requireValue(clientKey && clientSecret,
    'TikTok Accounts API refresh requires the app ID and app secret.')
  const result = await request(`${TIKTOK}/tt_user/oauth2/refresh_token/`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      client_id: clientKey, client_secret: clientSecret, grant_type: 'refresh_token', refresh_token: refreshToken
    })
  }, ctx)
  const accessToken = requireValue(result.data?.access_token,
    'TikTok Accounts API did not return an access token.')
  const businessId = requireValue(result.data?.open_id || directBusinessId,
    'TikTok Accounts API did not return the authorized account ID.')
  const auth = { accessToken, businessId,
    expiresAt: Date.now() + Math.max(60, Number(result.data?.expires_in || 86400)) * 1000 }
  tiktokTokenCache.set(clientId, auth)
  return auth
}

function tiktokPublishedAt(value) {
...
async function publishedPosts(account, ctx) {
  ...
  if (account.platform === 'meta') {
    let pageId = social.facebookPageId || envFor('FACEBOOK_PAGE_ID', id)
    let instagramId = social.instagramAccountId || envFor('INSTAGRAM_ACCOUNT_ID', id)
    const pageUsername = social.facebookPageUsername
    requireValue(pageId || instagramId || pageUsername, 'Connect this client’s confirmed Facebook Page or Instagram professional account to read published posts. Ad-account access alone does not identify the social profile.')
    ...
    // A confirmed public handle identifies the Page but does not grant access. Ask the
    // per-client Page token who it belongs to, then verify the exact configured handle;
    // never search an agency-wide Page list by display name.
    if (!pageId && pageUsername) {
      const profile = await request(url(`${GRAPH}/me`, {
        fields: 'id,name,username,instagram_business_account{id,username}'
      }), { headers: bearer(token) }, ctx)
      if (String(profile.username || '').toLowerCase() !== String(pageUsername).toLowerCase()) {
        throw new Error(`The configured Page token does not belong to @${pageUsername}.`)
      }
      pageId = requireValue(profile.id, `The configured Facebook Page @${pageUsername} could not be resolved.`)
      instagramId ||= profile.instagram_business_account?.id || ''
    }
  ...
  if (account.platform === 'tiktok') {
    // Prefer the Accounts API attached to the existing TikTok for Business app. Its
    // account token is separate from the advertiser token and refreshes every day.
    const businessAuth = await getTikTokBusinessContentAuth(id, ctx)
    if (businessAuth) {
      const posts = []
      let cursor = new Date(endExclusive(ctx.endDate)).getTime()
      for (let page = 0; page < MAX_PAGES; page++) {
        const data = await request(url(`${TIKTOK}/business/video/list/`, {
          business_id: businessAuth.businessId,
          fields: JSON.stringify(['item_id', 'thumbnail_url', 'share_url', 'embed_url', 'caption', 'media_type',
            'create_time', 'likes', 'comments', 'shares', 'video_views', 'reach']),
          cursor: String(cursor), max_count: '20'
        }), { headers: { 'Access-Token': businessAuth.accessToken } }, ctx)
        const videos = data.data?.videos || []
        posts.push(...videos.map((p) => ({
          id: p.item_id, title: p.caption || 'TikTok post', channel: 'TikTok',
          publishedAt: tiktokPublishedAt(p.create_time), url: p.share_url || p.embed_url,
          thumbnailUrl: p.thumbnail_url, views: num(p.video_views), reach: num(p.reach),
          engagements: sum(p.likes, p.comments, p.shares), metricNote: 'Lifetime platform metrics'
        })))
        if (!data.data?.has_more || Number(data.data.cursor) < new Date(ctx.startDate).getTime()) break
    ...
    // Retain support for an independently authorized Display API profile token.
    const token = requireValue(envFor('TIKTOK_CONTENT_ACCESS_TOKEN', id),
      'Authorize this client’s TikTok profile through the Accounts API or Display API.')
    const posts = []
    let cursor = new Date(endExclusive(ctx.endDate)).getTime()
    for (let page = 0; page < MAX_PAGES; page++) {
  ...
  if (account.platform === 'linkedin') {
    let organization = social.linkedinOrganizationId || envFor('LINKEDIN_ORGANIZATION_ID', id)
    const vanityName = social.linkedinVanityName
    requireValue(organization || vanityName,
      'Connect this client’s LinkedIn organization ID or confirmed company vanity name. The token also needs r_organization_social to read company posts.')
    requireValue(process.env.LINKEDIN_ACCESS_TOKEN, 'LinkedIn authorization is missing.')
    if (!organization && vanityName) {
      const lookup = await request(buildLinkedInUrl('/rest/organizations', {
        q: 'vanityName', vanityName
      }), { headers: liHeaders() }, ctx)
      const exact = (lookup.elements || []).find((item) =>
        String(item.vanityName || '').toLowerCase() === String(vanityName).toLowerCase())
      organization = requireValue(exact?.id,
        `The configured LinkedIn company ${vanityName} could not be resolved.`)
    }
    const posts = []
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      try {
        ...
        try {
          ...
          if (kind === 'posts') {
            ...
            const mappedMetaProfile = getClientById(account.clientId)?.socialAccounts?.facebookPageUsername
            const mappedLinkedInProfile = getClientById(account.clientId)?.socialAccounts?.linkedinVanityName
            message = ({ meta: mappedMetaProfile
              ? `The confirmed Meta profile @${mappedMetaProfile} is mapped. Authorize META_PAGE_ACCESS_TOKEN_${key} with Page/Instagram read access.`
              : `Confirm FACEBOOK_PAGE_ID_${key} / INSTAGRAM_ACCOUNT_ID_${key} and a Page read token.`,
              tiktok: `Authorize the client profile for TikTok Accounts API media access and set TIKTOK_CONTENT_REFRESH_TOKEN_${key}.`,
              linkedin: mappedLinkedInProfile
                ? `The confirmed LinkedIn company ${mappedLinkedInProfile} is mapped. Reauthorize LINKEDIN_ACCESS_TOKEN with r_organization_social and company access.`
                : `Set LINKEDIN_ORGANIZATION_ID_${key} and authorize r_organization_social.`,
