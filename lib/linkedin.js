...
export async function getLinkedInData({ clientId, range = '30d' } = {}) {
  ...
  const dateRange = encodeURIComponent(toDateRangeParam(start, end))
  const accountUrnEncoded = encodeURIComponent(`urn:li:sponsoredAccount:${client.linkedinAccountId}`)
