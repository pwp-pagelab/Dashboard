...
function getDateRange(range, client) {
  ...
  if (customRange) {
    ...
    startDate.setTime(new Date(client?.platformStartDates?.linkedin || client?.reportingStartDate || REPORTING_START_DATE).getTime())
  } else {
...
export { buildUrl as buildLinkedInUrl }
...
function sumAnalytics(elements = []) {
  return elements.reduce(
    (acc, row) => {
      ...
      acc.oneClickLeads += safeNum(row.oneClickLeads)
      acc.contactInfoShares += safeNum(row.leadGenerationMailContactInfoShares)
      return acc
    },
    {
      impressions: 0,
  ...
  return elements
    ...
    .map((row) => {
      ...
      const leadBreakdown = createLeadBreakdown({
        formSubmissions: Math.max(
          safeNum(row.oneClickLeads),
          safeNum(row.leadGenerationMailContactInfoShares)
        )
      })
...
function toDashboardRow({ client, totals, source }) {
  ...
  const leadBreakdown = createLeadBreakdown({
    formSubmissions: Math.max(totals.oneClickLeads, totals.contactInfoShares)
  })
  ...
  return {
    ...
    conversionBreakdown: {
      ...
      totalResults: totals.conversions
    },
    currencyCode: client?.currencyCode || 'SAR',
    conversionLabel: 'Website conversions',
