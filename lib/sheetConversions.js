...
export async function getSheetConversions(
  ...
  async function fetchPublicTab(tab) {
    const params = new URLSearchParams({ tqx: 'out:csv', sheet: tab.sheetName })
    const url = `https://docs.google.com/spreadsheets/d/${encodeURIComponent(config.spreadsheetId)}/gviz/tq?${params.toString()}`
