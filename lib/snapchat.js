...
export async function getSnapchatData({ clientId, range = '30d' } = {}) {
  ...
  const statsResp = await fetch(
    `https://adsapi.snapchat.com/v1/adaccounts/${adAccount.id}/stats?granularity=TOTAL&report_dimension=account&lifetime=${mapRangeToSnap(range)}`,
    {
      headers: {
        Authorization: `Bearer ${accessToken}`
      }
  ...
  if (!statsResp.ok) {
    throw new Error(statsJson?.error?.message || 'Failed to fetch Snapchat stats')
  }

  const timeseries = statsJson?.timeseries_stats || []
