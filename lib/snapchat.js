import { getClientById } from '../data/clients.js'
import { parseCustomDateRange } from './reportRange.js'

const REPORTING_START_DATE = '2026-01-01'

export async function getSnapAccessToken({ signal = AbortSignal.timeout(12000) } = {}) {
  const clientId = process.env.SNAP_CLIENT_ID
  const clientSecret = process.env.SNAP_CLIENT_SECRET
  const refreshToken = process.env.SNAP_REFRESH_TOKEN

  if (!clientId || !clientSecret || !refreshToken) {
    return null
  }

  const tokenResp = await fetch('https://accounts.snapchat.com/login/oauth2/access_token', {
    signal,
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded'
    },
    body: new URLSearchParams({
      grant_type: 'refresh_token',
      client_id: clientId,
      client_secret: clientSecret,
      refresh_token: refreshToken
    })
  })

  const text = await tokenResp.text()

  let json
  try {
    json = JSON.parse(text)
  } catch {
    throw new Error(`Snap OAuth returned non-JSON response: ${text.slice(0, 300)}`)
  }

  if (!tokenResp.ok || !json.access_token) {
    throw new Error(json.error_description || json.error || 'Failed to refresh Snapchat access token')
  }

  return json.access_token
}

function findMatchingSnapOrganization(organizations, client) {
  if (!client?.snapMatch || !Array.isArray(organizations)) return null

  const { type, value } = client.snapMatch
  const target = String(value || '').toLowerCase()

  if (type === 'includes') {
    return (
      organizations.find((item) =>
        String(item?.organization?.name || '').toLowerCase().includes(target)
      ) || null
    )
  }

  if (type === 'exact') {
    return (
      organizations.find(
        (item) => String(item?.organization?.name || '').toLowerCase() === target
      ) || null
    )
  }

  return null
}

function getSnapDateRange(range, client = null) {
  const now = new Date()
  now.setMinutes(0, 0, 0)

  const end = new Date(now)
  const start = new Date(now)
  const customRange = parseCustomDateRange(range, now)

  if (customRange) {
    start.setTime(new Date(`${customRange.startDate}T00:00:00.000Z`).getTime())
    end.setTime(new Date(`${customRange.endDate}T00:00:00.000Z`).getTime())
    end.setUTCDate(end.getUTCDate() + 1)
  } else if (range === '7d') {
    start.setDate(start.getDate() - 7)
  } else if (range === 'this_month') {
    start.setDate(1)
    start.setHours(0, 0, 0, 0)
  } else if (range === 'max') {
    start.setTime(new Date(client?.platformStartDates?.snapchat || client?.reportingStartDate || REPORTING_START_DATE).getTime())
    start.setMinutes(0, 0, 0)
  } else {
    start.setDate(start.getDate() - 30)
  }

  start.setMinutes(0, 0, 0)

  return {
    start_time: start.toISOString(),
    end_time: end.toISOString()
  }
}

export async function getSnapchatData({ clientId, range = '30d', adAccountId = null, accountName = null } = {}) {
  const client = getClientById(clientId)
  const effectiveAdAccountId = adAccountId || client?.snapchatAdAccountId || null
  const effectiveAccountName = accountName || client?.snapchatAdAccountName || null

  if (!client || (!effectiveAdAccountId && !client.platforms?.snapchat?.enabled)) {
    return null
  }

  const accessToken = await getSnapAccessToken()
  if (!accessToken) {
    return null
  }

  let matchedOrg = null
  let adAccount = effectiveAdAccountId
    ? {
        id: effectiveAdAccountId,
        name: effectiveAccountName || client.name
      }
    : null

  if (!adAccount) {
    const orgResp = await fetch(
      'https://adsapi.snapchat.com/v1/me/organizations?with_ad_accounts=true',
      {
        headers: {
          Authorization: `Bearer ${accessToken}`
        }
      }
    )

    const orgText = await orgResp.text()

    let orgJson
    try {
      orgJson = JSON.parse(orgText)
    } catch {
      throw new Error(`Snap organizations returned non-JSON response: ${orgText.slice(0, 300)}`)
    }

    if (!orgResp.ok) {
      throw new Error(orgJson?.error?.message || 'Failed to fetch Snapchat organizations')
    }

    const organizations = Array.isArray(orgJson?.organizations) ? orgJson.organizations : []
    const matchedOrgWrapper = findMatchingSnapOrganization(organizations, client)
    matchedOrg = matchedOrgWrapper?.organization || null

    if (!matchedOrg) {
      return null
    }

    adAccount = Array.isArray(matchedOrg.ad_accounts) ? matchedOrg.ad_accounts[0] : null
  }

  if (!adAccount?.id) {
    return null
  }

  const { start_time, end_time } = getSnapDateRange(range, client)

  const statsUrl =
    `https://adsapi.snapchat.com/v1/adaccounts/${adAccount.id}/stats` +
    `?granularity=TOTAL` +
    `&breakdown=campaign` +
    `&fields=impressions,swipes,spend,conversion_purchases` +
    `&start_time=${encodeURIComponent(start_time)}` +
    `&end_time=${encodeURIComponent(end_time)}`

  const statsResp = await fetch(statsUrl, {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  })

  const statsText = await statsResp.text()

  let statsJson
  try {
    statsJson = JSON.parse(statsText)
  } catch {
    throw new Error(`Snap stats returned non-JSON response: ${statsText.slice(0, 300)}`)
  }

  if (!statsResp.ok) {
    throw new Error(
      `Failed to fetch Snapchat stats: ${JSON.stringify(statsJson).slice(0, 1000)}`
    )
  }

  const totalStats = Array.isArray(statsJson?.total_stats) ? statsJson.total_stats : []
  const campaignBreakdowns =
    totalStats[0]?.total_stat?.breakdown_stats?.campaign || []

  if (!Array.isArray(campaignBreakdowns) || campaignBreakdowns.length === 0) {
    return {
      platform: 'Snapchat',
      campaign: effectiveAccountName || matchedOrg?.name || adAccount.name || 'Snapchat',
      currencyCode: client?.snapchatCurrencyCode || client?.currencyCode || 'SAR',
      conversionLabel: 'Purchases',
      spend: 0,
      impressions: 0,
      clicks: 0,
      ctr: 0,
      cpc: 0,
      conversions: 0
    }
  }

  const aggregated = campaignBreakdowns.reduce(
    (acc, item) => {
      const stats = item?.stats || {}
      acc.impressions += Number(stats.impressions || 0)
      acc.swipes += Number(stats.swipes || 0)
      acc.spendMicros += Number(stats.spend || 0)
      acc.conversions += Number(stats.conversion_purchases || 0)
      return acc
    },
    { impressions: 0, swipes: 0, spendMicros: 0, conversions: 0 }
  )

  const impressions = aggregated.impressions
  const swipes = aggregated.swipes
  const spend = aggregated.spendMicros / 1_000_000
  const conversions = aggregated.conversions
  const ctr = impressions > 0 ? (swipes / impressions) * 100 : 0
  const cpc = swipes > 0 ? spend / swipes : 0

  return {
    platform: 'Snapchat',
    campaign: effectiveAccountName || matchedOrg?.name || adAccount.name || 'Snapchat',
    currencyCode: client?.snapchatCurrencyCode || client?.currencyCode || 'SAR',
    conversionLabel: 'Purchases',
    spend,
    impressions,
    clicks: swipes,
    ctr,
    cpc,
    conversions
  }
}
