...
function getRangeEnd(range, now = new Date()) {
  const customRange = parseCustomDateRange(range, now)
  if (customRange) return customRange.endDate
...
export function applyMetaImportToDashboard(data, metaImport, { range = '30d', platform = 'all', now = new Date() } = {}) {
  ...
  const rangedRows = metaImport.rows.filter((row) => (
    !row.date || ((!rangeStart || row.date >= rangeStart) && (!rangeEnd || row.date <= rangeEnd))
  ))
