import { createLeadBreakdown, summarizeGoogleLeadActions } from './leads.js'
...
export async function getGoogleAccessToken({ signal = AbortSignal.timeout(12000) } = {}) {
...
export async function runGoogleQuery(customerId, accessToken, developerToken, loginCustomerId, query, { signal = AbortSignal.timeout(12000) } = {}) {
  const headers = {
...
`
  ...
  let dailyChunks = []
  let conversionActionChunks = []
  let leadMetricChunks = []
  let dailyLeadMetricChunks = []
  try {
  ...
  try {
    [conversionActionChunks, leadMetricChunks, dailyLeadMetricChunks] = await Promise.all([
      runGoogleQuery(customerId, accessToken, developerToken, loginCustomerId, conversionActionQuery),
      runGoogleQuery(customerId, accessToken, developerToken, loginCustomerId, leadMetricsQuery),
      runGoogleQuery(customerId, accessToken, developerToken, loginCustomerId, dailyLeadMetricsQuery)
    ])
  } catch {
    conversionActionChunks = []
    leadMetricChunks = []
    dailyLeadMetricChunks = []
  }
  ...
  const conversionActionRows = flattenResults(conversionActionChunks)
  const leadMetricRows = flattenResults(leadMetricChunks)
  const dailyLeadMetricRows = flattenResults(dailyLeadMetricChunks)
  ...
  const leadBreakdown = summarizeGoogleLeadActions(leadMetricRows, conversionActionRows)
  const spendByDate = new Map(
    dailyRows
      .filter((row) => row.segments?.date)
      .map((row) => [row.segments.date, microsToCurrency(row.metrics?.costMicros)])
  )
  const dailyLeadRows = new Map()
  dailyLeadMetricRows.forEach((row) => {
    const date = row.segments?.date
    if (!date) return
    const rows = dailyLeadRows.get(date) || []
    rows.push(row)
    dailyLeadRows.set(date, rows)
  })
  const dailyDates = new Set([...spendByDate.keys(), ...dailyLeadRows.keys()])
  const daily = Array.from(dailyDates)
    .sort((a, b) => a.localeCompare(b))
    .map((date) => {
      const spend = spendByDate.get(date) || 0
      const leadsForDay = summarizeGoogleLeadActions(dailyLeadRows.get(date) || [], conversionActionRows)
      return {
        date,
        spend,
        conversions: leadsForDay.totalLeads,
        formSubmissions: leadsForDay.formSubmissions,
        directMessages: leadsForDay.directMessages,
        totalLeads: leadsForDay.totalLeads,
        cpa: leadsForDay.totalLeads > 0 ? spend / leadsForDay.totalLeads : null
      }
    })
