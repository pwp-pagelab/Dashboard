...
export async function getGoogleAdsData({ dateRange, startDate, endDate } = {}) {
  ...
  const [snapshotChunks, keywordChunks, searchTermChunks] = await Promise.all([
  ...
  const keywordTable = keywordRows.map((row) => {
    ...
    return {
      ...
      cpa: conversions ? microsToCurrency(m.costPerConversion) : null,
      searchImpressionShare: pct(m.searchImpressionShare),
      lostIsBudget: pct(m.searchBudgetLostImpressionShare),
      lostIsRank: pct(m.searchRankLostImpressionShare)
    }
  })
  ...
  const searchTermsTable = searchTermRows.map((row) => {
    ...
    return {
      ...
      cost: microsToCurrency(m.costMicros),
      conversions: safeNum(m.conversions),
      cpa: safeNum(m.conversions) ? microsToCurrency(m.costPerConversion) : null
    }
  })
  ...
  if (conversions > 0 && microsToCurrency(metrics.costPerConversion) <= 0) {
    efficiencyStatus = 'Check conversion value mapping'
  } else if (conversions === 0 && cost > 0) {
  ...
  return {
    ...
    snapshot: {
      ...
      conversions,
      cpa: conversions ? microsToCurrency(metrics.costPerConversion) : null
    },
    visibility: {
      searchImpressionShare,
      lostIsBudget,
      lostIsRank,
      topImpressionShare: pct(metrics.searchTopImpressionShare),
      absoluteTopImpressionShare: pct(metrics.searchAbsoluteTopImpressionShare)
    },
    keywordHealth: {
      avgQualityScore,
